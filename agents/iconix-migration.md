...
IDs are stable — if the file is regenerated incrementally, preserve existing IDs and only
assign new IDs to newly discovered rules.
...
1. Append matched Precondition / Transition guard / Authorization / Workflow rules to
   ...
   ```
   Use the exact BR-ID assigned in Step 3. Skip if an existing precondition already covers
   the same entity and constraint.
...
2. Add `## Business rules cross-reference (Phase 5d)` at the bottom of the UC-DRAFT:
   ...
   | BR-005 | Authorization | Requires <role> | <file:line> | EXTRACTED |
   | BR-001 | Invariant | <description> | <construct> | EXTRACTED |
   | BR-004 | Calculation | <formula> | <file:line> | INFERRED |
   ```
