---
name: iconix-metrics
description: Use to produce metrics snapshots for the project — throughput, cycle time, gate-failure rates, drift, process compliance — as audit-friendly markdown plus machine-readable JSON. Snapshot-based; no external state. Read-only on artifacts.
...
- `trace_comment_coverage_pct` — invoke `.ci/validate-traceability.sh --full-scan`, parse the output for "OK (N files checked)" and "MISSING_TRACE" lines. Coverage = (N - missing) / N. When running from the meta-project, `ARTIFACT_ROOT` defaults to `.` (no env var needed). Note: the script checks that source files under `src/` and `tests/` carry a `Traceability:` comment citing valid artifact IDs; it does NOT validate artifact-to-artifact chains (REQ → UC → RB — those are covered by the orphan report). In multi-repo mode, `--full-scan` covers only tracked files in the meta-project's `src/` and `tests/`; service-repo source-file coverage is checked by each service repo's own CI using `ICONIX_CONFIG_PATH` (see `templates/git-integration/generic/validate-traceability.sh`).
