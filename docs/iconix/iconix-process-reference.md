...
_Last reviewed: v0.9.11 (Round 2 forcing-function fixes — M1 gate review uncovered six template/agent gaps. Group A: UC template gains `Invokes:` and `Domain entities introduced or used:` traceability fields, multi-state Postconditions; REQ template's `Source:` aligned to `Intakes:` for symmetry with UC. Group B: PO agent rule 12 added — cross-UC invocations must cite `<PREFIX>-UC-XXX | <Title> | <Package>` in both UC text and Traceability `Invokes:`; PO M1 checklist gains a mirror-consistency check; Traceability validation gains check #14 (invocation drift = M1 blocker). **No status shifts** — Ch3 #7 (two-column format), Ch4 #1 (8 easy steps), Ch3 #2 (UCs in context of object model — UC↔domain-entity tracing) all already ✅; v0.9.11 enriches their kit-location citations. v0.9.10 entries unchanged._
