...
_Last reviewed: v0.9.13 (Round 2 forcing-function fixes — M2 Analyst phase. Eight clarifications across `iconix-analyst.md` and `templates/robustness-template.puml`: UI sub-elements explicitly excluded from boundaries; Display vs Load controller distinction formalized; invoked UCs use `usecase` nodes (not `control`s) with rationale; Analyst's stale "initial domain model" wording corrected to "refinement" (PO has owned the initial draft since v0.9.3); RB template rewritten from macros to native PlantUML keywords with concrete actor-name placeholder; PDR readiness checklist gains UI-sub-element check, Invokes-mirror check, and a note that "every-sentence-maps-to-element" is human-verified not automated. **No status shifts.** Cited rules: Ch5 Robustness Top 10 #5 (syntax rules), Ch5 #1 (no GUI on domain model), Ch6 PDR #2 (no detailed design on RB) — all already ✅; v0.9.13 enriches kit-location citations. v0.9.11 entries unchanged. (v0.9.12 was tooling-only — `/iconix-upgrade` dogfood fixes.)_
