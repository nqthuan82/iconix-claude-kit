...
_Last reviewed: v0.9.19 (Round 6 — first **real M3 Developer** forcing-function run; **largest finding count of any round** (12). Developer agent had zero prior prompt review; every issue is net-new. Twelve fixes across three groups. **SD-rendering rules** (3 fixes): new section formalizing how v0.9.15's Invokes / UI-dependencies / Downstream-consumers sub-categories render at the SD level (synthetic-boundary + note for invoked UCs; `<<from PREFIX-UC-XXX>>` stereotype on lifeline for UI deps; dashed `..>` from producing entity to consumer-actor lifeline; framework-helper lifelines included only when non-trivial). **Template / convention gaps** (4 fixes): sequence template rewrite with stack-anchored multi-line lifeline labels and design-pattern demonstrations (Repository, DI interface) — replacing generic `User`/`ScreenName` placeholders that violated PO rules; new `templates/class-model-template.puml` (detailed static model — distinct from PO/Analyst's domain model); new `templates/cdr-report-template.md` per-UC M3 readiness report. **Allocation / structure rules** (5 fixes): rule 2 disambiguates ICONIX RB-controller from framework-controller (both exist in real stacks; one becomes messages, the other a lifeline); rule 4 adds explicit handling for architectural / DI interfaces with required justification; new behavior-allocation override — cross-cutting concerns from container-mapping override information-expert default; new "Code skeleton paths align with package-map" rule connecting Architect package-map to Developer source-file layout; rule 6 "stable SD" criteria made concrete (6 specific signals). CDR readiness checklist expanded to enforce all new rules. **No status shifts.** Cited rules: Ch8 Top 10 (sequence diagrams and behavior allocation), Ch9 CDR, Ch10 Implementation #1 (alternate courses), Ch11 #6 (cross-cutting concerns). v0.9.18 entries unchanged._
