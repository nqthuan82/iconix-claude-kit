...
_Last reviewed: v0.9.17 (Round 4 — first **real M2 Analyst** forcing-function run, producing fresh BS-RB-001 against BS-UC-001 + PO-only initial domain model; diffed against `06-BS-RB-001-...example.puml`. Six issues only visible by producing artifacts: Analyst rendering rules added for v0.9.15's three Invokes sub-categories (UI dependencies → `<<from ...>>` stereotype on boundary; downstream consumers → dashed `..>` arrow from produced entity); new `# Controller granularity` section mirrors PO rule 13 at the controller level (consolidate similar error responses; default to merge); workflow step 6 reworded from imperative-rewrite to verify-then-rewrite-only-if; Analyst Domain model rule 5 gains explicit `' VERIFY:` resolution procedure (PO writes; Analyst resolves at M2 by either keeping with `' RESOLVED at M2:` or removing if state/value); PDR readiness gains four mirror-checks (Invokes / UI deps / Downstream consumers / VERIFY-resolved). Note: example RB violates v0.9.13 verb-led controller rule (uses question-form `Is X OK?`) — known issue from prompt-review M2-#6; deferred to example refresh. **No status shifts.** Cited rules: Ch5 Top 10 #5 (RB syntax), Ch5 controller-as-action principle, Ch6 PDR. v0.9.15 entries unchanged. (v0.9.16 was tooling-only — PlantUML CI validation.)_
