...
_Last reviewed: v0.9.15 (Round 3 — first **real** forcing-function run, producing fresh M1 artifacts and diffing them against the worked example. Seven issues only visible by actually producing fresh output: REQ template gains `Related NFRs:` and `Related BRs:` fields the example used but the kit never standardized; REQ acceptance-criteria checkbox lifecycle now documented (Tester ticks per TC pass; PO confirms at merge); domain model template gains "ownership over time" header and `' VERIFY:` convention for PO-introduced ambiguities (mirrors intake `[VERIFY]`); UC template's `Invokes:` field split into three sub-fields (`Invokes (UC calls)`, `UI dependencies`, `Downstream consumers`) that distinguish three meaningfully different cross-UC dependencies; UC template's `Domain entities` field split into `introduced` vs `used` so the Analyst sees which entities are net-new at M2; PO rule 13 added codifying basic-course row granularity (one row per controller-equivalent system step). **No status shifts.** Cited rules: Ch3 #4 (UC in context of object model), Ch3 #7 (event/response flow), Ch4 #5 (REQ traced to UCs), Ch5 #1 (no GUI on domain model). v0.9.14 entries unchanged._
