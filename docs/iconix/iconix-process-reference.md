...
_Last reviewed: v0.9.14 (Round 2 forcing-function fixes — M2 Architect phase. Ten issues fixed: 5 new templates created (container-mapping, nfr-annotations, nfr-catalog, architecture-package-map, integration-surface) closing the under-template gap that left 4 of 5 Architect outputs format-unspecified; agent-prompt clarifications around UC-package-vs-architecture-package terminology, NFR-catalog input path, concurrent-touch resolver hand-offs for UC text edits; PDR readiness checklist expanded to enforce all 5 Architect artifacts and a "no blocking architectural questions without a Proposed ADR" item per Decision rule 5. **No status shifts.** Cited rules: Ch7 Top 10 (architecture decisions documented; cross-cutting concerns enumerated; testability seams), Ch6 PDR — all already ✅/⚠️; v0.9.14 enriches kit-location citations. v0.9.13 entries unchanged._
