...
| 5 | Use a traceability matrix | ✅ | `iconix-traceability.md` + `test-matrix.md`; Tester `# Test matrix lifecycle` owns creation (Phase 7, after first TC batch) and updates (Phase 9 / bug-fix flows). Fixed v0.9.22 — creation trigger was missing, only update paths existed. |
...
_Last reviewed: v1.0.33 (Migration Phase 5c extensions v1.0.24–v1.0.29 + Phase 5d v1.0.32–v1.0.33 — Migration agent reverse-engineering capability expansion. **Phase 5c extended:** cross-stack schema detection across 7 language families (C# EF Core, Java JPA/Hibernate, Python Django/SQLAlchemy, PHP Doctrine/Eloquent, Ruby ActiveRecord, TypeScript TypeORM/Prisma, Go GORM/Ent); Track C adds single-source-of-truth files (`schema.prisma`, `db/schema.rb`, `ent/schema/*.go`) and migration DSL fallback (Alembic, Liquibase, Laravel, Rails); Track B5 resolves integer status columns by searching application-layer enums/constants outside entity classes (7-language pattern table, 4 confidence tiers). Skip logic decoupled: domain-glossary.md (Steps 1–3) always runs when any schema source is detected, regardless of `stack.bdd`; BDD-DRAFT generation (Steps 4–6) gates on `stack.bdd: true` and UC-DRAFT existence. **Phase 5d (new):** extracts business rules from 4 tracks — S (schema glossary pull-through), V (validator annotations, language-aware), D (domain-layer guard clauses, specification/policy classes, calculation methods), T (SQL triggers) — and classifies into 7 categories (Invariant, Precondition, Postcondition, Transition guard, Calculation, Authorization, Workflow) → `migration/business-rules.md`. Step 4 auto-annotates UC-DRAFT `## Preconditions` with matched rules and adds a `## Business rules cross-reference` table per UC-DRAFT. Theory audit: **Ch2 #4** (domain model as project glossary ✅) — `domain-glossary.md` + `business-rules.md` together serve as the reverse-engineered domain vocabulary for legacy projects; no status shift. **Ch12 #4** (scenario-level acceptance testing ✅) — Phase 5c BDD-DRAFTs and Phase 5d business rules both feed into Tester's UC scenario coverage; no status shift. **Ch13 #9** (link requirements to use cases ✅) — Phase 5d Step 4 auto-links business rules to UC-DRAFT preconditions, strengthening the REQ→UC chain in migration mode; no status shift. No coverage table row changes — all are kit extensions within the Migration agent's reverse-engineering role, reinforcing existing ✅ rows rather than closing ❌ gaps in the ICONIX book rules.) Last reviewed: v1.0.23 (Migration Phase 5c — BDD Gherkin scenario synthesis from SQL schema. New phase in `iconix-migration.md` for both graph-assisted and code-walking modes. New outputs: `features/BDD-DRAFT-*.feature` and `migration/domain-glossary.md`. Theory audit: Ch12 #4 (scenario-level acceptance testing ✅) — Phase 5c produces reverse-engineered DRAFT inputs for Tester's M3 acceptance test generation; Ch2 #4 (domain model as project glossary ✅) — `migration/domain-glossary.md` is the reverse-engineered equivalent built from SQL table definitions. No rule status shifts. Summary Coverage Matrix unchanged. v1.0.22 entries unchanged.) Last reviewed: v0.9.24 (Legacy-code integration guidance — kit extension across Analyst / Architect / Developer. Analyst: Boundary stereotype split into Inbound / Outbound sub-categories; new `# Outbound Boundary — legacy code and external systems` section; PDR checklist updated. Architect: Decision rule 7 added — when a new UC touches a legacy class that violates ICONIX rules, treat it as external dependency; raise ADR; map Adapter to Infrastructure container. Developer: rule 9 added — Outbound Boundary wrapping legacy code → SD lifeline is the Adapter interface; legacy class in a `note over` block, never as a lifeline. Theory audit: Ch5 #7 (Boundary object naming ✅) — Outbound Boundary is a clarification within existing coverage; Ch7 #5 (TA grounded in requirements ✅) — ADR requirement reinforces this; Ch8 Four Essential Steps #2 (objects from RB become SD lifelines ✅) — interface-as-lifeline is consistent with existing rule. No status shifts. PDF unavailable on this machine; matrix used as sole authority.) Last reviewed: v0.9.23 (Phase 9 cycle log creation trigger fix — kit extension: `iconix-orchestrator.md` Phase 9 routing had `phase9-cycles/UC-XXX-cycle.md` referenced only as an "optional: append final entry" at 9.4. No agent was told to CREATE the file at 9.1 or to append iteration rows at 9.2. Fixed: Orchestrator 9.1 now creates the file from template; 9.2 appends a row after each Reviewer verdict; 9.4 fills the Exit section. Template header updated to match. No Rosenberg rule row affected — kit extension. v0.9.22 entries unchanged.) Last reviewed: v0.9.22 (test-matrix creation trigger fix — Ch12 #5 "Use a traceability matrix": `iconix-tester.md` had the template referenced in the artifacts list and Phase 9 update paths, but no explicit instruction to CREATE the matrix during Phase 7 after first TC batch. Added `# Test matrix lifecycle` section with create / extend / update steps; updated coverage-gates heading from "run before release" to "run before M3 gate". No status shift — ✅ was aspirational but is now correctly enforced. Cited rule: Ch12 #5. v0.9.21 entries unchanged.) (Round 8 — first **real Phase 9** forcing-function walkthrough across all three new Reviewer modes (Pre-merge drift / Bug-fix verification / Type 2 closure). 9 issues only visible by producing artifacts for each mode end-to-end. **Pre-merge drift mode** (3): new `[INFO]` finding-tag formalized with severity table; `## Bug triage` section made conditional (omit when not a bug-triage invocation); phase9-cycle template's "Reviewer verdict" column standardized to the four discrete tokens (`APPROVE` / `APPROVE WITH NOTES` / `REQUEST CHANGES` / `BLOCK MERGE`) so `iconix-metrics` can parse iteration-count distributions. **Bug-fix verification mode** (2): regression-sweep step added explicitly (cite Tester's regression-sweep result; no silent skipping); Reviewer must populate the original bug report's `## Closure` section (not just the verification report). **Type 2 closure mode** (3): `Driven by CI report:` field added to Closure schema (preserves audit chain "what triggered this REQ-change"); PR-comment posting clarified — closure verdict is a comment on the SAME implementation PR, not a separate PR; bug-report template's Traceability vs Closure ownership distinguished (Traceability filled at triage; Closure filled at verification/closure). **Cross-cutting** (1): new `RE-TRIAGE` recommendation in Bug-fix verification mode — the in-the-wild case where a Type 1 fix attempt reveals the SD is the actual root cause (bug was mis-triaged); routes back to Bug triage with the new context. **No status shifts.** Cited rules: Ch11 #1 (Model Update at every gate), Ch10 #9 (if coding reveals design wrong, change it AND review the process — Type 2 closure mode is the "review the process" half), Ch11 #5 (follow up review with action points). v0.9.20 entries unchanged._
