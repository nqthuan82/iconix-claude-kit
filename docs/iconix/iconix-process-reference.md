...
_Last reviewed: v0.9.18 (Round 5 — first **real M2 Architect** forcing-function run, producing all 5 Architect artifact categories for BS-UC-001; diffed against the example's BS-ADR-001 (the only Architect artifact the example ships) plus stack info from `iconix.config.example.yaml`. Ten issues only visible by producing artifacts: container-mapping testability column gains "(out of scope — covered via upstream)" convention for indirect seams; new Traceability check #15 closes the per-UC NFR-list 3-place-duplication gap (catalog ↔ container-mapping ↔ nfr-annotations); nfr-annotations "Out-of-scope NFRs" framing made size-aware (small vs large catalogs); package-map gains `Infrastructure (external)` layer for cross-team / infra-owned containers (e.g., PendingReviewsQueue owned by INFRA-88); integration-surface drops redundant "Failure handling" column (kept per-touchpoint sub-sections), bidirectional table moved to commented optional block; container-mapping "Open architectural questions" formalized to `<question>. [Proposed ADR-XXX]` format; nfr-catalog `Owner:` field split into `Defined by:` + `Enforced by:` for split-ownership NFRs; new `templates/milestone-report-template.md` formalizes M1/M2/M3 readiness format with machine-readable Recommendation token and gate-specific check sub-sections (was format-unspecified — same gap Architect had pre-v0.9.14). **No status shifts.** Cited rules: Ch7 Top 10 (architecture decisions documented; cross-cutting concerns; testability seams), Ch6 PDR, Ch4 #5 (REQ traceability — extended via NFR-list check). v0.9.17 entries unchanged._
