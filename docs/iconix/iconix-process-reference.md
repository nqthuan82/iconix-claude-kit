...
_Last reviewed: v0.9.10 (Forcing-function fixes from real-world test run on the Write Customer Review example. Seven clarifications across PO agent prompt + intake/UC templates: multi-input intake convergence rule, REQ atomicity heuristic, initial domain model template + inlined critical heuristics, conditional path fork convention, "two paragraphs" → "one page" clarification (resolves a contradiction between the agent prompt and the UC template), input/output banner separators on email and transcript intake templates, Status-Ready enforcement before extraction. **No status shifts** — all changes clarify or refine existing ✅ rules. Cited rules: Ch3 #7 (two-column UC format), Ch3 #1 (UC brevity — one page / two paragraphs typical), Ch2 #3 (initial domain model before UCs), Ch4 #1 (8 easy steps to better use case). v0.9.8 and earlier entries unchanged. (v0.9.9 was a tooling-only release — `/iconix-upgrade`.)_
